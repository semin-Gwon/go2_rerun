...
- [Unitree-Go2-Robot](https://github.com/Unitree-Go2-Robot) organization 패키지: `go2_bringup`, `go2_description`, `go2_driver`, `go2_interfaces`, `go2_robot`, `go2_rviz`, `unitree_api`, `unitree_go`
- [abizovnuralem/go2_ros2_sdk](https://github.com/abizovnuralem/go2_ros2_sdk)

## 라이선스

이 저장소는 커스텀 시각화 코드, 외부 ROS2 패키지, 로봇 모델 에셋을 함께 포함합니다.

- 이 저장소의 커스텀 코드(`map_success.py` 포함)는 아직 별도의 루트 라이선스를 달지 않았습니다.
- `src/` 아래 외부 패키지는 각 원본 upstream 라이선스를 따릅니다.
- `go2_description/` 아래 로봇 모델 에셋은 Unitree 또는 원본 upstream 출처의 라이선스를 별도로 확인해야 할 수 있습니다.

개인 포트폴리오 범위를 넘어 재배포하거나 활용하기 전에는 upstream 라이선스를 확인하고, 필요하다면 커스텀 코드에 대한 루트 `LICENSE` 파일을 추가하는 것이 좋습니다.
