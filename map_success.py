...
# [Fix] Explicitly add unitree_go to python path
sys.path.append("/home/jnu/go2_ws/install/unitree_go/local/lib/python3.10/dist-packages")

# [Fix] Manually load the C Typesupport library
try:
    ctypes.CDLL("/home/jnu/go2_ws/install/unitree_go/lib/libunitree_go__rosidl_typesupport_introspection_c.so")
except OSError as e:
    print(f"Warning: Could not load C typesupport library: {e}")
...
class RerunImageNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("🚀 로봇 시각화 노드 시작 중 (map_success 수정본)...")
        
        # [중요] 멀티스레딩을 위한 콜백 그룹 (병렬 처리 허용)
        ...
        # 토픽 구독 (이름과 QoS 확인)
        self.create_subscription(CompressedImage, "/my_go2/color/image_raw/compressed", self.rgb_callback, qos_profile_sensor_data, callback_group=self.cb_group)
        self.create_subscription(Image, "/my_go2/depth/image_rect_raw", self.depth_callback, qos_profile_sensor_data, callback_group=self.cb_group)
        self.create_subscription(PointCloud2, "/utlidar/cloud", self.lidar_callback, lidar_qos, callback_group=self.cb_group)
        self.create_subscription(Odometry, "/utlidar/robot_odom", self.odom_callback, 10, callback_group=self.cb_group)
        
        # [최종] /lf/lowstate (20Hz) - 네트워크 대역폭 한계로 인해 이것만 수신 가능함이 증명됨
        self.create_subscription(LowState, "/lf/lowstate", self.lowstate_callback, 10, callback_group=self.cb_group)
        ...
        self.create_timer(1.0, self.timer_callback, callback_group=self.cb_group)
    ...
    def setup_leg_meshes(self):
        ...
        base_path = "/home/jnu/go2_ws/go2_description/assets"
    ...
    def update_leg_chain(self, leg_name, q1, q2, q3):
        # [Fix] Rerun RotationAxisAngle Type Error Fix
        # Use simple Axis-Angle to Quaternion conversion
        def q_from_axis_angle(axis, angle):
            # axis: tuple/list (x,y,z), angle: rad
            th = float(angle) / 2.0
            s = np.sin(th)
            return [axis[0]*s, axis[1]*s, axis[2]*s, np.cos(th)] # xyzw

        # 1. Base -> Hip
        cfg = self.legs[leg_name]
        # Hip Joint: X-axis
        q_hip = q_from_axis_angle([1.0, 0.0, 0.0], q1)
        rr.log(
            f"world/robot/{leg_name}_hip",
            rr.Transform3D(
                translation=list(cfg["offset"]),
                rotation=rr.Quaternion(xyzw=q_hip)
            )
        )
        ...
        if cfg["mirror"]: thigh_offset[1] *= -1 
        
        q_thigh = q_from_axis_angle([0.0, 1.0, 0.0], q2)
        rr.log(
            f"world/robot/{leg_name}_hip/{leg_name}_thigh",
            rr.Transform3D(
                translation=list(thigh_offset),
                rotation=rr.Quaternion(xyzw=q_thigh)
            )
        )

        # 3. Thigh -> Calf
        # Calf Joint: Y-axis
        q_calf = q_from_axis_angle([0.0, 1.0, 0.0], q3)
        rr.log(
            f"world/robot/{leg_name}_hip/{leg_name}_thigh/{leg_name}_calf",
            rr.Transform3D(
                translation=list(self.d_calf),
                rotation=rr.Quaternion(xyzw=q_calf)
            )
        )
